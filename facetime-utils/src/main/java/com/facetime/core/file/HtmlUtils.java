...
public class HtmlUtils {
	 ...
	 * 
	 * @param strMap
	 *            key为select选项的值，value为select显示的内容
	 * @param selectKey
	 *            选中项的key
	 * @return 经过html敏感字符处理的html代码
	 */
	public static final String createSelectOptions(Map<String, Object> strMap,
			String selectKey) {
	...
	/**
	 * 当要把字符串正确显示在网页上
	 * 或者设置控件的值例如： <input type="text" name="txt"
	 * value="fdsfsd&lt;sf&gt;">
	 * 替换字符串里面跟HTML标签冲突的字符
	 * 去掉字符串的前后空格 如果字符串为null,返回空串
	 * 
	 * @param strInput
	 *            要显示的字符串
	 * @return 处理后的字符串
	...
	public static final String splitRow(String strContent, String symbol) {
		...
		StringBuilder sbf = new StringBuilder();
		String[] aryStr = StringUtils.toArray(strContent, symbol);
