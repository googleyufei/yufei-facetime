...
/**
 * Given a (growing) set of URLs, can periodically validate to see if any of the underlying resources has changed. This
 ...
 * net.street.common.resource caching -- that's how granular they get with their "If-Modified-Since", "Last-Modified" and "Expires"
 * headers.
 */
public class URLChangeTracker {
	...
	private final ClasspathURLConverter<URL> classpathURLConverter;
	 ...
	 * 
	 * @param url
	 *            of the net.street.common.resource to add, or null if not known
	...
	public long add(URL url) {
		...
		try {
			resourceFile = new File(converted.toURI());
		} catch (URISyntaxException ex) {
			resourceFile = new File(converted.getPath());
		}
