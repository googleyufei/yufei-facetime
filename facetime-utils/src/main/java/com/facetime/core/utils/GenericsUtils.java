...
public class GenericsUtils {
	/**
	 * Get the class represented by the reflected type. This method is lossy;
	 * You cannot recover the type information from the class that is returned.
	 * <p/>
	 ...
	 * is defined with upper bounds then {@code Object} is returned.
	 * 
	 * @param actualType
	...
	public static Class asClass(Type actualType) {
		...
		if (actualType instanceof ParameterizedType) {
			...
			// The sun implementation returns getRawType as Class<?>, but there
			// is room in the interface for it to be
			// some other Type. We'll assume it's a Class.
			// TODO: consider logging or throwing our own exception for that day
			// when "something else" causes some confusion
		...
		if (actualType instanceof WildcardType) {
			...
			// If there is no lower bounds then the only thing that makes sense
			// is Object.
			return Object.class;
		}
	 ...
	 * Class that is represented by the method's generic return type. Any
	 * parameter information in the generic return type is lost.
	 * 
	 ...
	 * Class that is represented by the field's generic type. Any parameter
	 * information in the generic type is lost, if you want to preserve the type
	 * parameters of the return type consider using
	 ...
	 * @return the class represented by the field's generic type, resolved based
	 *         on the containingClass.
	 ...
	 * Analyzes the method in the context of containingClass and returns the
	 * Class that is represented by the method's generic return type. Any
	 * parameter information in the generic return type is lost. If you want to
	 * preserve the type parameters of the return type consider using
	...
	public static boolean isAssignableFrom(Type suspectedSuperType, Type suspectedSubType) {
		...
		// We need to figure out what the generic parameters in the targetClass
		// are
		// as they pertain to the sourceType.

		if (suspectedSuperType instanceof WildcardType) {
			// ? extends Number
			// needs to match all the bounds (there will only be upper bounds or
			// lower bounds
				...
				if (suspectedSuperType instanceof ParameterizedType) {
					if (resolved instanceof ParameterizedType) {
						...
						for (int i = 0; i < type1Arguments.length; ++i)
							if (!isAssignableFrom(type1Arguments[i], type2Arguments[i]))
								return false;
						return true;
					}
				} else if (suspectedSuperType instanceof GenericArrayType)
					if (resolved instanceof GenericArrayType)
						return isAssignableFrom(((GenericArrayType) suspectedSuperType).getGenericComponentType(),
	 ...
	 * argument resolved form the class hierarchy. This may be something other
	 * than a simple Class if the type argument is a ParameterizedType for
	 * instance (e.g. List&lt;E>; List&lt;Map&lt;Long, String>>, E would be
	 * returned as a ParameterizedType with the raw type Map and type arguments
	 * Long and String.
	 * <p/>
	 * 
	 * @param type
	 ...
	 * the code that would be used to define the type.
	 * 
	 * @param type
	 *            - the type.
	...
	static String toString(WildcardType wt) {
		...
		return String.format("? %s %s", isSuper ? "super" : "extends", isSuper ? toString(wt.getLowerBounds())
				: toString(wt.getLowerBounds()));
	}

	/**
	 * Find the index of the TypeVariable in the classes parameters. The offset
	 * can be used on a subclass to find the actual type.
	 * 
	 * @param typeVariable
	 *            - the type variable in question.
	 * @return the index of the type variable in it's declaring
	...
	private static int getTypeVariableIndex(final TypeVariable typeVariable) {
		...
		for (int typeArgumentIndex = 0; typeArgumentIndex < typeParameters.length; typeArgumentIndex++)
			// The .equals for TypeVariable may not be compatible, a name check
		...
		throw new RuntimeException(String.format("%s does not have a TypeVariable matching %s",
	 ...
	 * @param containingType
	 *            - the shallowest type in the hierarchy where type is defined.
	 * @return the passed type if nothing to resolve or a copy of the type with
	 *         the type arguments resolved.
	 */
	...
	/**
	 * @param types
	 *            - Array of types to resolve. The unresolved type is replaced
	 *            in the array with the resolved type.
	 * @param containingType
	 *            - the shallowest type in the hierarchy where type is defined.
	...
	/**
	 * Method to resolve a TypeVariable to it's most <a href=
	 * "http://java.sun.com/docs/books/jls/third_edition/html/typesValues.html#112582"
	 * >reifiable</a> form.
	 * <p/>
	 * <p/>
	 * How to resolve a TypeVariable:<br/>
	 * All of the TypeVariables defined by a generic class will be given a Type
	 * by any class that extends it. The Type given may or may not be reifiable;
	 * it may be another TypeVariable for instance.
	 * <p/>
	 * Consider <br/>
	 ...
	 * the TypeVariable "A". We can do that by first finding the index of "A" in
	 * the Pair.class.getTypeParameters() array of TypeVariables.
	 * <p/>
	 * To get to the Type provided by StringLongPair you access the generics
	 * information by calling StringLongPair.class.getGenericSuperclass; this
	 * will be a ParameterizedType. ParameterizedType gives you access to the
	 * actual type arguments provided to Pair by StringLongPair. The array is in
	 * the same order as the array in Pair.class.getTypeParameters so you can
	 * use the index we discovered earlier to extract the Type; String.class.
	 * <p/>
	 * When extracting Types we only have to consider the superclass hierarchy
	 * and not the interfaces implemented by the class. When a class implements
	 * a generic interface it must provide types for the interface and any
	 * generic methods implemented from the interface will be re-defined by the
	 * class with it's generic type variables.
	 * 
	 * @param typeVariable
	...
	private static Type resolve(TypeVariable typeVariable, Type containingType) {
		...
		// If you pass a List<Long> as the containingType then the TypeVariable
		// is going to be resolved by the
		...
		while (genericSuperclass != null && // true for interfaces with no superclass
				!theClass.equals(Object.class) && !theClass.equals(typeVariableOwner)) {
	 ...
	 *            - the shallowest type in the hierarchy where type is defined.
	 * @return the passed type if nothing to resolve or a copy of the type with
	 *         the upper and lower bounds resolved.
	 */
