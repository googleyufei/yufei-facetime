...
public class LunarDate {
	...
	// 农历 y年的总天数
	private int yearDays(final int y) {
		int i, sum = 348;
	...
	// 农历 y年闰月的天数
	private int leapDays(final int y) {
		if (leapMonth(y) != 0) {
	...
	// 农历 y年m月的总天数
	private int monthDays(final int y, final int m) {
	...
	public String toString() {
		...
		sb.append(_leap ? "闰" : "").append(chineseNumber[_month - 1]).append("月");
		sb.append(getChinaDayString(_day));
		return sb.toString();
	}
}
