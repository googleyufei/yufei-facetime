 ...
 * method (or methods) that "locks down" the creating, to prevent it from being
...
public class OneShotLock {
	...
	private void innerCheck() {
		if (lock) {
			// The depth to find the caller of the validate() or lock() method
			// varies between JDKs.
			...
			throw new IllegalStateException(String.format("Method %s may no longer be invoked.", elements[i + 2]));
		}
	}
