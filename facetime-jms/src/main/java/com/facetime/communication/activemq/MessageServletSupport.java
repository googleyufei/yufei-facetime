...
public abstract class MessageServletSupport extends HttpServlet {
	...
	public void init(ServletConfig servletConfig) throws ServletException {
		...
		if (facetimeLogger.isDebugEnabled()) {
			facetimeLogger.debug("Defaulting to use topics: " + defaultTopicFlag);
		}
		...
		AmqConsumer.initContext(getServletContext());
	}
	...
	protected void appendParametersToMessage(HttpServletRequest request, TextMessage message) throws JMSException {
		...
		Map<Object, Object> parameters = new HashMap<Object, Object>(parameterMap);
		...
		for (Iterator<?> iter = parameters.entrySet().iterator(); iter.hasNext();) {
			...
			if (!destinationParameter.equals(name) && !typeParameter.equals(name) && !bodyParameter.equals(name)
					&& !"JMSDeliveryMode".equals(name) && !"JMSPriority".equals(name) && !"JMSTimeToLive".equals(name)) {
				...
				if (value instanceof Object[]) {
					...
					if (array.length == 1) {
						...
						facetimeLogger.warn("Can't use property: " + name + " which is of type: "
								+ value.getClass().getName() + " value");
						value = null;
						int size = array.length;
						for (int i = 0; i < size; i++) {
							facetimeLogger.debug("value[" + i + "] = " + array[i]);
						}
					}
				}
				if (value != null) {
	 ...
	 */
	protected Destination getDestination(AmqConsumer client, HttpServletRequest request) throws JMSException {
	 ...
	 */
	protected Destination getDestinationFromURI(AmqConsumer client, HttpServletRequest request) throws JMSException {
		...
		uri = uri.replace('/', '.');
		facetimeLogger.debug("destination uri=" + uri);
	 ...
	 */
	protected Destination getDestination(AmqConsumer client, HttpServletRequest request, String destinationName)
			throws JMSException {
		...
		facetimeLogger.debug(destinationName + " (" + (isTopic ? "topic" : "queue") + ")");
		if (isTopic) {
	...
	protected String getSelector(HttpServletRequest request) throws IOException {
		return request.getHeader(AmqConsumer.selectorName);
	}
}
